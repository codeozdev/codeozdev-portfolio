      ...
      <div
        ...
        {propTabs.map((tab, idx) => (
          <button
            ...
            className={cn('relative md:rounded-full md:px-4 md:py-2', tabClassName)}
            style={{
            ...
            {active.value === tab.value && (
              <motion.div
                ...
                className={cn(
                  'absolute inset-0 md:rounded-full bg-gray-200 dark:bg-zinc-800',
                  activeTabClassName
                )}
              />
            )}

            <span className='relative block text-black dark:text-white text-sm md:text-base p-2 w-40 md:w-auto'>
              {tab.title}
            </span>
          </button>
        ))}
